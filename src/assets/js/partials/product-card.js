...
import { getCachedProductDetails, quickView } from '../utils';
...
class ProductCard extends HTMLElement {
  ...
  async onReady(){
      ...
      this.loadProductDetails().then(() => this.render())
  }
  ...
  async loadProductDetails() {
    ...
    try {
      const details = await getCachedProductDetails(this.product.id);
      this.product = {
        ...this.product,
  ...
  render(){
    ...
    this.effectiveStatus = (this.product.is_out_of_stock && window.notify_when_available_in_card && !['donating', 'financial_support'].includes(this.product?.type))
      ...
      : this.product.status;
      this.innerHTML = `
        <div class="relative group ${!this.fullImage ? 's-product-card-image' : 's-product-card-image-full'}">
        <a href="${this.product?.url}" class="product-image-wrapper"
          aria-label="${this.escapeHTML(this.product?.image?.alt || this.product.name)}">

            <div class="product-image-inner ${this.secondImage ? "has-hover" : ""}">

              <!-- MAIN IMAGE -->
              <img 
                class="s-product-card-image-${salla.url.is_placeholder(this.mainImage)
                  ? 'contain'
                  : this.fitImageHeight || 'cover'} product-img front"
                src="${this.mainImage || this.product?.thumbnail || this.placeholder || ''}"
        ...
        <div class="s-product-card-content">
          ...
          ? `<a class="!leading-[24px] text-secondary text-[10px] md:text-xs font-bold" href="${this.product.category.url}">
                ${this.product.category.name}
            </a>`
          : ''}
      ...
      `
    this.querySelector('.quick-view-button')?.addEventListener('click', () => {
      quickView.open(this.product);
    });
